...
def detect_ppe(photo, bucket,folder_name):

    session = boto3.Session(profile_name='default')
    client = session.client('rekognition')

    response = client.detect_protective_equipment(Image={'S3Object': {'Bucket': bucket, 'Name': folder_name + '/' + photo}},
...
def main():
    ...
    person_count = detect_ppe(photo, bucket,folder_name)
